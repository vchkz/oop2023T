...
//                    img->image->_data[p * 4 + b] = std::min(255, img->image->_data[p * 4 + b] + layer->_data[p * 4 + b]);
//                }
...
////    glTexImage2D(GL_TEXTURE_2D, 0, format, img->image->_size[0], img->image->_size[1], 0, format, GL_UNSIGNED_BYTE, img->image->_layers[0]->_data);
